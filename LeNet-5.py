...
train_file = 'E:/train.txt'
val_file = 'E:/val.txt'
test_file = 'E:/test.txt'
learning_rate = 0.001
num_epochs = 10
batch_size = 1
dropout_rate = 0.5
...
val_generator = ImageDataGenerator(class_list=val_file, n_class=num_class, batch_size=1, shuffle=False,
                                   flip=False)
test_generator = ImageDataGenerator(class_list=test_file, n_class=num_class, batch_size=1, shuffle=False,
                                    flip=False)
for i in range(num_epochs):
    ...
    train_step.run(feed_dict={x: batch_xs, y_: batch_ys, keep_prob: dropout_rate})

# 在测试数据上测试准确率
print("test accuracy %g" % accuracy.eval(feed_dict={x: batch_xs, y_: batch_ys, keep_prob: 1.0}))
